...
JOINT_LIMITS_CONFIG_BY_ATTACHMENT = {
    "arm": "config/joint_limits.yaml",
    "northrop_arm": "config/joint_limits_jn.yaml",
}
...
def launch_setup(context, *args, **kwargs):
    ...
    attachment = LaunchConfiguration('attachment', default='arm').perform(context)
    hardware_type = LaunchConfiguration('hardware_type', default='main').perform(context)

    joint_limits_file = JOINT_LIMITS_CONFIG_BY_ATTACHMENT.get(attachment)
    if joint_limits_file is None:
        valid_attachments = ", ".join(sorted(JOINT_LIMITS_CONFIG_BY_ATTACHMENT.keys()))
        raise RuntimeError(
            f"Unsupported rover arm attachment '{attachment}'. "
            f"Expected one of: {valid_attachments}"
        )
